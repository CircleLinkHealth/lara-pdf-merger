...
class TCPDF_FONTS
{
    ...
    public static function _getTrueTypeFontSubset($font, $subsetchars)
    {
        ...
        if (0x10000 != TCPDF_STATIC::_getULONG($font, $offset)) {
        ...
        if (0x5F0F3CF5 != TCPDF_STATIC::_getULONG($font, $offset)) {
        ...
        $short_offset = (0 == TCPDF_STATIC::_getSHORT($font, $offset));
        $offset += 2;
    ...
    public static function addTTFfont($fontfile, $fonttype = '', $enc = '', $flags = 32, $outpath = '', $platid = 3, $encid = 1, $addcbbox = false, $link = false)
    {
        ...
        if (empty($fonttype)) {
            if (0x10000 == TCPDF_STATIC::_getULONG($font, 0)) {
                // True Type (Unicode or not)
                $fonttype = 'TrueTypeUnicode';
            } elseif ('OTTO' == substr($font, 0, 4)) {
        ...
        $fmetric['diff'] = '';
        if (('TrueType' == $fmetric['type']) or ('Type1' == $fmetric['type'])) {
            if ( ! empty($enc) and ('cp1252' != $enc) and isset(TCPDF_FONT_DATA::$encmap[$enc])) {
        ...
        if ('Type1' == $fmetric['type']) {
            ...
            $offset += 2;
            $isFixedPitch = (0 == TCPDF_STATIC::_getULONG($font, $offset)) ? false : true;
            $offset += 2;
            if ($isFixedPitch) {
                $fmetric['Flags'] |= 1;
            }
        ...
        if (('TrueTypeUnicode' == $fmetric['type']) and (256 == count($ctg))) {
            $fmetric['type'] = 'TrueType';
        }
    ...
    public static function getFontFullPath($file, $fontdir = false)
    {
        ...
        if ((false !== $fontdir) and @file_exists($fontdir.$file)) {
    ...
    public static function getUniord($uch)
    {
        ...
        for ($i = 0; $i < $length; ++$i) {
            ...
            if (0 == $countbytes) { // get starting octect
                if ($char <= 0x7F) {
                    return $char; // use the character "as is" because is ASCII
                }
                if (0x06 == ($char >> 0x05)) { // 2 bytes character (0x06 = 110 BIN)
    ...
    public static function utf8Bidi($ta, $str = '', $forcertl = false, $isunicode = true, &$currentfont)
    {
        ...
        for ($i = 0; $i < $numchars; ++$i) {
            if (($levcount > 0) and (($i + 1) < $numchars) and ($chardata[($i + 1)]['level'] == $prevlevel)) {
                if (('N' == $chardata[$i]['type']) and ('L' == $chardata[($i - 1)]['type']) and ('L' == $chardata[($i + 1)]['type'])) {
                    $chardata[$i]['type'] = 'L';
                } elseif (('N' == $chardata[$i]['type']) and
                 (('R' == $chardata[($i - 1)]['type']) or ('EN' == $chardata[($i - 1)]['type']) or ('AN' == $chardata[($i - 1)]['type'])) and
                 (('R' == $chardata[($i + 1)]['type']) or ('EN' == $chardata[($i + 1)]['type']) or ('AN' == $chardata[($i + 1)]['type']))) {
                    $chardata[$i]['type'] = 'R';
                ...
                if (('N' == $chardata[$i]['type']) and ('L' == $chardata[$i]['sor']) and ('L' == $chardata[($i + 1)]['type'])) {
                 ...
                 (('R' == $chardata[$i]['sor']) or ('EN' == $chardata[$i]['sor']) or ('AN' == $chardata[$i]['sor'])) and
                 (('R' == $chardata[($i + 1)]['type']) or ('EN' == $chardata[($i + 1)]['type']) or ('AN' == $chardata[($i + 1)]['type']))) {
                    $chardata[$i]['type'] = 'R';
                } elseif ('N' == $chardata[$i]['type']) {
                    // N2. Any remaining neutrals take the embedding direction
                ...
                if (('N' == $chardata[$i]['type']) and ('L' == $chardata[($i - 1)]['type']) and ('L' == $chardata[$i]['eor'])) {
                 ...
                 (('R' == $chardata[($i - 1)]['type']) or ('EN' == $chardata[($i - 1)]['type']) or ('AN' == $chardata[($i - 1)]['type'])) and
                 (('R' == $chardata[$i]['eor']) or ('EN' == $chardata[$i]['eor']) or ('AN' == $chardata[$i]['eor']))) {
                    $chardata[$i]['type'] = 'R';
                } elseif ('N' == $chardata[$i]['type']) {
                    // N2. Any remaining neutrals take the embedding direction
                    $chardata[$i]['type'] = $chardata[$i]['sor'];
        ...
        for ($i = 0; $i < $numchars; ++$i) {
            if (('B' == $chardata[$i]['type']) or ('S' == $chardata[$i]['type'])) {
                ...
                $j = $i + 1;
                while ($j < $numchars) {
                    if ((('B' == $chardata[$j]['type']) or ('S' == $chardata[$j]['type'])) or
                        (($j == ($numchars - 1)) and ('WS' == $chardata[$j]['type']))) {
                        $chardata[$i]['level'] = $pel;
                        break;
                    }
                    if ('WS' != $chardata[$j]['type']) {
                        break;
                    }
                    ++$j;
                }
        ...
        if ($arabic) {
            ...
            $x           = 0;
            for ($i = 0; $i < $numchars; ++$i) {
                if (('AL' == TCPDF_FONT_DATA::$uni_type[$chardata[$i]['char']]) or (32 == $chardata[$i]['char']) or (8204 == $chardata[$i]['char'])) {
                    $charAL[$x]        = $chardata[$i];
                    $charAL[$x]['i']   = $i;
                    $chardata[$i]['x'] = $x;
                    ++$x;
            ...
            for ($i = 0; $i < $numchars; ++$i) {
                ...
                if ('AL' == TCPDF_FONT_DATA::$uni_type[$thischar['char']]) {
                    $x = $thischar['x'];
                    if ($x > 0) {
                        $prevchar = $charAL[($x - 1)];
                    } else {
                    ...
                    if ((false !== $prevchar) and (false !== $nextchar) and
                        (('AL' == TCPDF_FONT_DATA::$uni_type[$prevchar['char']]) or ('NSM' == TCPDF_FONT_DATA::$uni_type[$prevchar['char']])) and
                        (('AL' == TCPDF_FONT_DATA::$uni_type[$nextchar['char']]) or ('NSM' == TCPDF_FONT_DATA::$uni_type[$nextchar['char']])) and
                    ...
                    } elseif (((false !== $prevchar) and
                        (('AL' == TCPDF_FONT_DATA::$uni_type[$prevchar['char']]) or ('NSM' == TCPDF_FONT_DATA::$uni_type[$prevchar['char']])) and
                        ($prevchar['type'] == $thischar['type'])) or
                        ((false !== $nextchar) and (1567 == $nextchar['char']))) {
                        // final
                        if (($i > 1) and (1607 == $thischar['char']) and
                            (1604 == $chardata[$i - 1]['char']) and
                            (1604 == $chardata[$i - 2]['char'])) {
                            //Allah Word
            ...
            for ($i = 0; $i < ($numchars - 1); ++$i) {
                if ((1617 == $chardata2[$i]['char']) and (isset(TCPDF_FONT_DATA::$uni_diacritics[($chardata2[$i + 1]['char'])]))) {
